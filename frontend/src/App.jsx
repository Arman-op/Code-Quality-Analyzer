...
function MissionDebrief({ scanResult }) {
  ...
  const exportPDF = async () => {
    ...
    if (element) {
      try {
        const canvas = await html2canvas(element, { backgroundColor: "#000008", useCORS: true, logging: false });
  ...
  return (
    <section id="report" style={{ padding: "100px 24px" }}>
      <div id="report-content" style={{ maxWidth: 1100, margin: "0 auto", padding: "40px", background: "var(--void)", borderRadius: "16px" }}>
        {downloading && <style>{`#report-content * { animation: none !important; transition: none !important; }`}</style>}
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          <div className="section-label" style={{ marginBottom: 12 }}>07 — MISSION DEBRIEF</div>
          <h2 style={{ fontFamily: "Orbitron", fontSize: 36, fontWeight: 700, color: "var(--cyan)" }}>Repository Health Report</h2>
